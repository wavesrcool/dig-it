...
export type TypesWebsModalPaymentAmount = {
 ...
 *
 */
export const WebsModalPaymentAmount: React.FC<TypesWebsModalPaymentAmount> = ({
  basis,
}: TypesWebsModalPaymentAmount) => {
  useTranslation(basis.dictionary);

  const fold = useFold();
  const WebsDigCreateShape = useShape(ofWebsDigCreateShape);

  const lcWebsModalPaymentAmountOnChange = React.useCallback(
    (value: string) => {
      //
      // @notes:
  ...
  return (
    <div className={`flex flex-row w-full`}>
      <div className={`flex flex-row items-center h-full w-12 -mr-12 pl-2`}>
        {WebsDigCreateShape.amountType ? (
          ...
          <p className={"font-mono font-bold text-lg pl-4"}>{`$`}</p>
        )}
      </div>
      <ComponentsInput
        basis={{
          ...basis,
          cl: `w-full rounded-full bg-base-200/50 font-semibold pl-12`,
          placeholder: ``,
          value: WebsDigCreateShape.amountBundle.letters,
          change: lcWebsModalPaymentAmountOnChange,
        }}
      />
    </div>
  );
