...
import { ofWebsDigCreateShape } from "@webs-shapes/webs/dig-create/WebsDigCreateShape";
  ...
  const lcaWebsPostComplete = React.useCallback(async () => {
    ...
    const run = async () => {
      try {
        ...
        await router.push(
          `account/${WebsDigCreateShape.bundlesContact.letters}`
        );
    ...
    // end
    return;
  }, [WebsDigCreateShape.bundlesContact.letters, router]);

  return (
    <div className={`flex flex-col w-full`}>
      <div className={`flex flex-row w-full pt-8 justify-center `}>
        <p className={"font-dongle font-medium text-4xl text-justify"}>
          {`${t(
            `glossary:your_dig_has_been_posted`,
            `your_dig_has_been_posted`
          )}!`}
        </p>
      </div>
